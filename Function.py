...
def two(prev_out):
    ...
    th_input = prev_out + " " + str(next_out2)
    return next_out2, th_input


def three(th_input):
    input_data = pd.read_csv('csvs/input_with_4_output.csv')
    input_data = input_data.values.astype(np.float32)

    output_data = pd.read_csv('csvs/distance.csv')
    output_data = output_data.values.astype(np.float32)
    ...
    class NeuralNetwork(torch.nn.Module):
        def __init__(self):
            super().__init__()
            self.fc1 = torch.nn.Linear(64, 255)
            self.fc2 = torch.nn.Linear(255, 5)
            self.relu = torch.nn.ReLU()
            self.sigmoid = torch.nn.Sigmoid()
